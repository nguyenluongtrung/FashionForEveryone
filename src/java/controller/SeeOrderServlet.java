...
public class SeeOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SeeOrderServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SeeOrderServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<OrderJoin> orderJoin = new OrderDAO().getAllOrderDetails(sellerID);
        request.setAttribute("orderJoin", orderJoin);
        int orderID = Integer.parseInt(request.getParameter("orderID"));
        String productName = request.getParameter("productName");
        
        OrderJoin order_info = new OrderDAO().getOrderJoinInfo(orderID, productName);
